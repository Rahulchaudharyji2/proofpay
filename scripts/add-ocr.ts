...
async function main() {
  console.log("Adding OCR service...");

  const ocrService = await prisma.service.upsert({
    where: { slug: "ocr" },
    update: {},
    create: {
  ...
  const providersData = [
    {
      ...
      description: "High speed text extraction.",
      metrics: { price: 0.05, quality: 92, reliability: 95, latencyMs: 1500 }
    },
    {
      name: "DocScan AI",
      address: ethers.Wallet.createRandom().address, 
      description: "Specialized in scanned handwritten documents.",
      metrics: { price: 0.08, quality: 88, reliability: 98, latencyMs: 800 }
    },
    {
      name: "OCRPro",
      address: ethers.Wallet.createRandom().address, 
      description: "Pro-grade extraction.",
      metrics: { price: 0.10, quality: 98, reliability: 99, latencyMs: 2000 }
    }
  ];
  ...
  for (const p of providersData) {
    ...
    await prisma.providerService.upsert({
      where: { providerId_serviceId: { providerId: provider.id, serviceId: ocrService.id } },
      update: {},
      create: { providerId: provider.id, serviceId: ocrService.id }
    });
  }
  console.log("Added OCR providers to DB.");
  ...
  for (const dbAgent of dbAgents) {
    ...
    for (const m of mandates) {
      if (!m.allowedServices.includes("ocr")) {
        await prisma.mandate.update({
          where: { id: m.id },
          data: { allowedServices: { push: "ocr" } }
        });
      }
    }
  }
  ...
  // For demo, we just don't restrict providers on the default agent, or we pass all
