...
async function main() {
  console.log("Adding Data Processing service...");

  const dataService = await prisma.service.upsert({
    ...
    create: {
      ...
      category: "COMPUTE",
      description: "Clean, format, and process large CSV/JSON datasets.",
      unit: "per megabyte",
      basePrice: 0.15,
    }
  });
  ...
  const providersData = [
    {
      ...
      description: "Enterprise grade data scrubbing.",
      metrics: { price: 0.20, quality: 99, reliability: 99, latencyMs: 3000 }
    },
    {
      name: "DataClean B",
      address: ethers.Wallet.createRandom().address, 
      description: "Fast parallel processing.",
      metrics: { price: 0.10, quality: 92, reliability: 95, latencyMs: 1200 }
    },
    {
      name: "DataClean C",
      address: ethers.Wallet.createRandom().address, 
      description: "Cost-effective batch cleaner.",
      metrics: { price: 0.05, quality: 85, reliability: 90, latencyMs: 5000 }
    }
  ];
  ...
  for (const p of providersData) {
    ...
    await prisma.providerService.upsert({
      where: { providerId_serviceId: { providerId: provider.id, serviceId: dataService.id } },
      update: {},
      create: { providerId: provider.id, serviceId: dataService.id }
    });
  }
  console.log("Added Data Processing providers to DB.");
  ...
  for (const dbAgent of dbAgents) {
    ...
    for (const m of mandates) {
      if (!m.allowedServices.includes("data-processing")) {
        await prisma.mandate.update({
          where: { id: m.id },
          data: { allowedServices: { push: "data-processing" } }
        });
      }
    }
  }
  ...
  const allowedServices = ["translation", "compute", "storage", "ai-inference", "summarization", "ocr", "image-generation", "file-conversion", "data-processing"];
